...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-1">Monitor and manage all payment transactions</p>
        </div>
                    ...
                    @forelse($payments as $payment)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">
                                    <div class="font-medium text-gray-900">{{ $payment->checkout_request_id ?? 'N/A' }}</div>
                                    @if($payment->mpesa_receipt_number)
                                        <div class="text-gray-500">{{ $payment->mpesa_receipt_number }}</div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="flex-shrink-0 h-8 w-8">
                                        <div class="h-8 w-8 rounded-full bg-gray-500 flex items-center justify-center">
                                            <span class="text-xs font-medium text-white">
                                                {{ $payment->user ? substr($payment->user->name, 0, 1) : 'U' }}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="ml-3">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">
                                    @if($payment->content_type === 'book')
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            Book
                                        </span>
                                    @elseif($payment->content_type === 'video')
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                            Video
                                        </span>
                                    @elseif($payment->content_type === 'membership')
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                            Membership
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                            {{ ucfirst($payment->content_type ?? 'Unknown') }}
                                        </span>
                                    @endif
                                    @if($payment->content_id)
                                        <div class="text-xs text-gray-500 mt-1">ID: {{ $payment->content_id }}</div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Payments will appear here when users make transactions.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
