...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-1">Manage user memberships and subscriptions</p>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">
                        {{ $users->where('membership_status', 'active')->where('membership_expires_at', '>', now())->count() }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">
                        {{ $users->where('membership_status', 'expired')->count() + $users->where('membership_expires_at', '<=', now())->count() }}
                    </p>
                </div>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($user->membership_expires_at)
                                    {{ $user->membership_expires_at->format('M d, Y') }}
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($user->membershipPayments->count() > 0)
                                    <div>
                                        <span class="font-medium">{{ $user->membershipPayments->count() }}</span> payments
                                        <div class="text-xs text-gray-500">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                <a href="{{ route('admin.memberships.details', $user) }}" 
                                ...
                                @if(!$user->hasMembership())
                                    <form method="POST" action="{{ route('admin.memberships.activate', $user) }}" class="inline">
                                        ...
                                        <button type="submit" 
                                                ...
                                                onclick="return confirm('Activate membership for this user?')">
                                            Activate
                                        </button>
                                    </form>
                                @endif
                                ...
                                @if($user->hasMembership())
                                    <form method="POST" action="{{ route('admin.memberships.suspend', $user) }}" class="inline">
                                        ...
                                        <button type="submit" 
                                                ...
                                                onclick="return confirm('Suspend membership for this user?')">
                                            Suspend
                                        </button>
                                    </form>
                                @endif
